---
title: 'voi.remote for Value of Information calculation: package overview'
...
vignette: >
 ...
 %\VignetteIndexEntry{voi.remote for Value of Information calculation: package overview}
...
This document is based on the VoI vignette written by Christopher Jackson. The `voi.remote` package has the exact same functionalities as `voi` package, but further allows users to run all the VoI functions on the server rather than running locally.
...
The example model is a model used for *decision making*, which has been the most common application of VoI, e.g. in health economic evaluations. A later section describes the use of VoI methods for a model that is used for *estimation* of uncertain quantities, rather than for explicit decision-making.
...
Most of the functions in the `voi` package work with a *random sample* of model inputs and outputs, generated from "uncertainty analysis", also known as "probabilistic sensitivity analysis". For the example model, these are simple to generate, as follows.
...
The `voi.remote` package contains a simple function `evpi` to compute the EVPI using the above procedure. The function automatically detects whether your outputs are in net benefit or cost-effectiveness format.
...
The function `evppi.remote` can be used to compute this.
...
To call `evppi.remote`, supply a sample of outputs and inputs (in the same form as defined above) in the first two arguments. The parameter or parameters of interest (whose EVPPI is desired) is supplied in the `"pars"` argument. This can be expressed in various ways.
...
The method can be changed by supplying the `method` argument to `evppi.remote`. Some methods have additional options to tune them. For a full list of these options, see `help(evppi.remote)`.
...
It is only applicable to calculating the multiparameter EVPPI for 2 or more parameters. FIXME why doesn't it work in this example?
    ...
    model_run(model_name="voi", api_key=<your_api_key>, model_input = list(outputs = outputs_nb, inputs = inputs, pars=c("p1","p2"), func="evppi", method="inla"))
...
A standard error for the EVPPI estimates from the GAM method, resulting from uncertainty about the parameters of the GAM approximation, can be obtained by calling `evppi.remote` with `se=TRUE`. This uses $B$ samples from the distribution of the GAM parameters, thus the standard error can be estimated more accurately by increasing B.

    evppi.remote(outputs_nb, inputs, pars="p1", se=TRUE, B=100)

#### Single-parameter methods

These are only applicable for computing the EVPPI for a single scalar parameter. They are supplied in the package for technical completeness, but for single-parameter EVPPI we have found it to be sufficiently reliable to use the default GAM method, which requires less tuning than these methods.
...
This method is available in the function `evppi_mc.remote`. It requires the user to supply two functions; one to evaluate the decision-analytic model, and one to generate parameter values.
...
These functions are then supplied as arguments to `evppi_mc.remote`, along with the number of samples to draw in the inner and outer loops. 1000 inner samples and 100 outer samples give a reasonable EVPPI estimate in this example, but many more samples may be required for the result to converge to the EVPPI in more complex models.
...
A function of this form should then be passed to `evppi_mc.remote` if the parameters are correlated. This allows `evppi_mc.remote` to draw from the appropriate distribution in the inner loop.
...
The function `evsi.remote` can be used to calculate this. The default method is based on nonparametric regression (from [Strong et al.](https://dx.doi.org/10.1177%2F0272989X15575286)). This requires the user to either
...
To illustrate how to use `evsi.remote`, suppose we want to collect a sample of $n$ normally-distributed observations in order to get a better estimate of the treatment 2 effectiveness $p_1$. Under current information, $p_1$ is distributed as $N(1,1)$. After collecting the sample, we would expect this distribution to become more precise, hence reduce the likelihood of making a wrong decision. The EVSI measures the expected improvement in net benefit from this sample.
...
The `evsi.remote` function can then be used to compute the EVSI for a series of different sample sizes from this design. Note how the EVSI converges to the EVPPI as the sample size increases.
...
The known standard deviation defaults to 1, but can be changed, e.g. to 2, by calling `evsi.remote` with an `aux_pars` argument, e.g. `evsi(..., aux_pars=list(sd=2), ...)`.

Note that the results will be slightly different every time the `evsi.remote` function is invoked with the same arguments, due to Monte Carlo error from generating the data (unless the seed is set with the R function `set_seed` before each invocation).
...
`"trial_binary"`: A two-arm trial with a binary outcome. Requires two parameters to be specified in `pars`: the probability of the outcome in arm 1 and 2 respectively. The sample size is the same in each arm, specifed in the `n` argument to `evsi.remote`, and the binomial outcomes are returned in the first and second column respectively.

### Importance sampling method

An alternative method comes from [Menzies (2015)](https://doi.org/10.1177/0272989X15583495) and is based on importance sampling. This can be invoked as `evsi.remote(..., method="is")`.
...
Again, this study model is available as a built-in study design, so instead of writing a user-defined likelihood and data generation function, `evsi.remote` can also be invoked with `study="normal_known", method="is"`.
...
The model *output* quantity is $p_2$, and the model *inputs* are $p_1$ and $\beta$. We now want to determine the expected value of further information. This is measured in terms of expected *reductions in variance* of $p_2$. This has a decision-theoretic interpretation, with "loss" being measured by squared error of a parameter estimate compared to the true parameter value. See [Jackson et al. 2019](https://doi.org/10.1080/01621459.2018.1562932).
...
These can be computed using nonparametric regression, as described in [Jackson et al. 2019](https://doi.org/10.1080/01621459.2018.1562932). This is implemented in the function `evppivar.remote` in the `voi.remote` package.
...
EVPPI is estimated here by *nonparametric regression* of the output on the input. Recall that the method of [Strong et al. (2013)](https://doi.org/10.1177/0272989X13505910) was used in `evppi.remote` for models with decisions. [Jackson et al. 2019](https://doi.org/10.1080/01621459.2018.1562932) showed how this method applied in a wider class of problems, including models for estimation.

To estimate the expected reduction in variance in $p_2$, given knowledge of $p_1$, the `evppivar.remote` function fits a regression model with `p2` as the outcome, and `p1` as the single predictor. A generalized additive model based on splines is fitted using the `gam` function from the `mgcv` package.
...
This agrees (up to Monte Carlo error) with the value produced by `evppivar.remote`, which is obtained by a closely-related method, as the variance of the fitted values from this regression. This is equal to the total variance minus the variance of the residuals, through the "law of total variance":
...
This is achieved using the function `evsivar.remote` in the `voi.remote` package, as follows.

-   As in `evppivar.remote`, samples from the prior distributions of the parameters are supplied in the data frame `inputs`.
...
Alternatively, suppose we were able to collect information about the *unbiased* population, whose infection prevalence is $p_2$. That is, suppose we surveyed $n$ individuals, each with a binary infection outcome generated with probability $p_2$. In this case, we can compute the EVSI by using `evsivar` with the model `inputs` defined to equal the model outputs, as follows:
